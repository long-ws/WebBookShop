	...
	<section class="section-content">
		<div class="container">
			<header class="section-heading py-4">
				<h3 class="section-title">Sửa người dùng
					#${requestScope.user.id}</h3>
			</header>
			<!-- section-heading.// -->

			<main class="row mb-5">
				<form class="col-lg-6" method="POST"
					action="${pageContext.request.contextPath}/admin/userManager/update">
					...
					<div class="mb-3">
						<label for="user-username" class="form-label">Tên đăng
                   ...
                   ? 'is-invalid' : (not empty requestScope.user.username ? 'is-valid' : '')}"
					...
					<div class="mb-3">
						<label for="user-password" class="form-label">Mật khẩu</label> <input
							type="password"
                   ...
                   ? 'is-invalid' : (not empty requestScope.user.password ? 'is-valid' : '')}"
					...
					<div class="mb-3">
						<label for="user-fullname" class="form-label">Họ và tên <span
                   ...
                   ? 'is-invalid' : (not empty requestScope.user.fullname ? 'is-valid' : '')}"
							id="user-fullname" name="fullname"
                   ...
                   ? 'is-invalid' : (not empty requestScope.user.email ? 'is-valid' : '')}"
							id="user-email" name="email" value="${requestScope.user.email}"
							required>
					...
					<div class="mb-3">
						<label for="user-phoneNumber" class="form-label">Số điện
                   ...
                   ? 'is-invalid' : (not empty requestScope.user.phoneNumber ? 'is-valid' : '')}"
							id="user-phoneNumber" name="phoneNumber"
							value="${requestScope.user.phoneNumber}" required>
						<c:if
					...
					<div class="mb-3">
						<label class="form-label d-block">Giới tính <span
                   ...
                   ? 'is-invalid' : (not empty requestScope.user.address ? 'is-valid' : '')}"
                    ...
                    ? 'is-invalid' : (not empty requestScope.user.role ? 'is-valid' : '')}"
							id="user-role" name="role" required>
						...
						<c:if test="${not empty requestScope.violations.roleViolations}">
							<div class="invalid-feedback">
								<ul class="list-unstyled">
									<c:forEach var="violation"
										items="${requestScope.violations.roleViolations}">
										<li>${violation}</li>
									</c:forEach>
								</ul>
							</div>

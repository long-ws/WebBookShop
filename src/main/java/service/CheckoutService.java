...
public class CheckoutService {
	...
	public ShippingInfo calculateShippingInfo(long cartId, long methodId, String provinceCode) {
		try {
			...
			ShippingCalculationResult calcResult = shippingCalculator.calculateFee(methodId, provinceCode, totalWeight,
					totalVolume);
	...
	public Payment checkoutFromCart(long userId, long cartId, int deliveryMethod, double deliveryPrice,
			String receiverName, String receiverPhone, String province, String district, String ward,
			String addressDetail, int estimatedDays, Long finalVoucherId, Long finalShipVoucherId, String customerNote)
			throws SQLException {
		...
		if (finalVoucherId != null && finalVoucherId > 0) {
			...
			if (discountVoucher != null && discountVoucher.isActive()
					&& cartTotal >= discountVoucher.getMinPurchase()) {
				...
				if (baseAmountForDiscount > 0) {
					if (discountVoucher.getCalculationMethod() == 1) {
						...
						discountOrderAmount = Math.min(calculated,
								discountVoucher.getMaxDiscount() > 0 ? discountVoucher.getMaxDiscount()
										: Double.MAX_VALUE);
					} else {
		...
		for (CartItem ci : items) {
			...
			orderItemDAO.insert(new OrderItem(0L, orderId, p.getId(), p.getPrice(), p.getDiscount(), ci.getQuantity(),
	...
	public Payment checkoutFromCart(long userId, long cartId, int deliveryMethod, double deliveryPrice,
			String receiverName, String receiverPhone, String province, String district, String ward,
			String addressDetail, int estimatedDays) throws SQLException {
		return checkoutFromCart(userId, cartId, deliveryMethod, deliveryPrice, receiverName, receiverPhone, province,
				district, ward, addressDetail, estimatedDays, null, null, null);
	}
	...
	public Payment checkoutFromCart(long userId, long cartId, int deliveryMethod, double deliveryPrice,
			String receiverName, String receiverPhone, String province, String district, String ward,
			String addressDetail) throws SQLException {
		return checkoutFromCart(userId, cartId, deliveryMethod, deliveryPrice, receiverName, receiverPhone, province,
				district, ward, addressDetail, 3, null, null, null);
	}

	/**
	 * Checkout với ShippingInfo (mới)
	 */
	public Payment checkoutWithShippingInfo(long userId, long cartId, ShippingInfo shippingInfo, String receiverName,
			String receiverPhone, String province, String district, String ward, String addressDetail)
			throws SQLException {

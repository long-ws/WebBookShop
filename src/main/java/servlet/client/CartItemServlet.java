...
public class CartItemServlet extends HttpServlet {
	...
	private void handleAdd(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		long cartId;
		if (cart == null) {
			cartId = cartService.insert(new Cart(0L, userId, LocalDateTime.now(), null));
		} else {
			cartId = cart.getId();
		}
		...
		if (item != null) {
			...
			cartItemService.insert(new CartItem(0L, cartId, productId, quantity, LocalDateTime.now(), null));
		}

		...
		<div class="container">
			...
			<main class="table-responsive-xl mb-5">
				<table
					...
					<tbody>
						<c:forEach var="productReview" varStatus="loop"
							...
							<tr>
								...
								<td><a
									href="${pageContext.request.contextPath}/product?id=${productReview.product.id}#review"
									target="_blank"> ${productReview.product.name} </a></td>
								<td class="text-center text-nowrap"><a
									...
									<div class="btn-group" role="group">
										<button type="submit"
											...
											form="update-hide-${productReview.id}"
											${productReview.isShow==0 ? 'disabled' : '' }>Ẩn</button>
										<button type="submit"
											class="btn ${productReview.isShow == 1 ? 'btn-secondary' : 'btn-warning'}"
											form="update-show-${productReview.id}"
											${productReview.isShow==1 ? 'disabled' : '' }>Hiện</button>
									</div>
									<form
										action="${pageContext.request.contextPath}/admin/reviewManager"
										method="post" id="update-hide-${productReview.id}">
									...
									<form
										action="${pageContext.request.contextPath}/admin/reviewManager"
										method="post" id="update-show-${productReview.id}">
										<input type="hidden" name="id" value="${productReview.id}">
										<input type="hidden" name="action" value="SHOW"> <input
											type="hidden" name="page" value="${requestScope.page}">
									</form></td>
							</tr>
						</c:forEach>

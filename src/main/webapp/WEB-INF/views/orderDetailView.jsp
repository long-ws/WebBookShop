...
<body>
	...
	<section class="section-pagetop bg-light">
		<div class="container">
			<h2 class="title-page">Thông tin đơn hàng
				#${requestScope.order.id}</h2>
		</div>
		<!-- container.// -->
	</section>
	<!-- section-pagetop.// -->
						...
						<main class="col-md-9">
							<article class="card mb-4">
								...
								<header class="card-header">
									<strong class="d-inline-block me-4">Mã đơn hàng:
										${requestScope.order.id}</strong> <span>Ngày mua:
										${requestScope.createdAt}</span>
									<c:choose>
										<c:when test="${requestScope.order.status == 1}">
											<span class="badge bg-warning text-dark float-end">Đang
												giao hàng</span>
										</c:when>
										<c:when test="${requestScope.order.status == 2}">
											<span class="badge bg-success float-end">Giao hàng
												thành công</span>
										</c:when>
								...
								<div class="card-body pb-0">
									<div class="row">
										<div class="col-lg-8">
											...
											<p class="lh-lg">
												${sessionScope.currentUser.fullname} <br> Số điện
												thoại: ${sessionScope.currentUser.phoneNumber} <br> Địa
												chỉ: ${sessionScope.currentUser.address}
											</p>
										</div>
										<div class="col-lg-4">
											<h6 class="text-muted">Hình thức thanh toán</h6>
											<span class="text-success"> <i
												class="fab fa-lg fa-cc-visa"></i>
												${requestScope.order.deliveryMethod == 1 ? "Giao tiêu chuẩn" : "Giao nhanh"}
											</span>
											<p class="lh-lg">
												Tạm tính:
												<fmt:formatNumber pattern="#,##0"
													value="${requestScope.tempPrice}" />
												₫ <br> Phí vận chuyển:
												<fmt:formatNumber pattern="#,##0"
													value="${requestScope.order.deliveryPrice}" />
												₫ <br> <strong> Tổng cộng: <fmt:formatNumber
														pattern="#,##0"
													...
													<td>
														<figure class="itemside">
															...
															<figcaption class="info">
																<a
																	href="${pageContext.request.contextPath}/product?id=${orderItem.product.id}"
																	target="_blank"> ${orderItem.product.name} </a>
															</figcaption>
														</figure>
													</td>
													<td>
															...
															<c:choose>
																<c:when test="${orderItem.discount == 0}">
																	<span class="price"> <fmt:formatNumber
																			pattern="#,##0" value="${orderItem.price}" />₫
																	</span>
																</c:when>
																<c:otherwise>
																	<div>
																		<span class="price"> <fmt:formatNumber
																			...
																			₫
																		</span> <span class="ms-2 badge bg-info"> - <fmt:formatNumber
																				pattern="#,##0" value="${orderItem.discount}" />%
																		</span>
																	</div>
											...
											<form action="${pageContext.request.contextPath}/orderDetail"
												...
												<c:choose>
													<c:when test="${empty requestScope.confirmCancel}">
														<button type="submit" name="action" value="requestCancel"
															class="btn btn-danger me-2">Hủy đơn hàng</button>
													</c:when>

													<c:otherwise>
														<span>Bạn có chắc chắn muốn hủy đơn?</span>
														<button type="submit" name="action" value="confirmCancel"
															class="btn btn-danger me-2">Xác nhận hủy</button>
														<button type="submit" name="action" value="cancelCancel"
															class="btn btn-secondary">Hủy thao tác</button>
													</c:otherwise>
												</c:choose>
											</form>
										</c:when>
										...
										<c:otherwise>
											<form action="${pageContext.request.contextPath}/cartItem"
												...
												<input type="hidden" name="action" value="add"> <input
													...
													value="${requestScope.orderItems[0].product.id}"> <input
													type="number" name="quantity"
													class="form-control w-auto d-inline" value="1" min="1"
													max="${requestScope.orderItems[0].product.quantity}"
													step="1">

												<button type="submit" class="btn btn-primary">Mua
													lại</button>
											</form>
										</c:otherwise>
									</c:choose>
